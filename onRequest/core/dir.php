...
class dir
{
	...
	public static function createFolder($dirname){
		//检测文件夹名称的合法性
		if(wpfString::checkFilename(basename($dirname))){
			//当前目录下是否存在同名文件夹名称
	...
	public static function renameFolder($oldname,$newname){
		//检测文件夹名称的合法性
		if( wpfString::checkFilename(basename($newname))){
			//检测当前目录下是否存在同名文件夹名称
	 ...
	 * @return string
	 */
	public static function cutFolder($src,$dst){
		//echo $src,"--",$dst;
		if(file_exists($dst)){
			if(is_dir($dst)){
